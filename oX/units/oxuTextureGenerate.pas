...
begin
   ...
   if(err) then begin
      ...
      if(imgProps.error.e <> 0) then
         addError('image: [' + sf(imgProps.error.e) + '] ' + eGetCodeName(imgProps.error.e));
...
begin
   ...
   if(Result = 0) then
      ...
      log.e('Failed to load image: ' + image.FileName + ' (' + eGetCodeName(Result) + ', ' + eGetCodeName(Image.error) + ', ' + f.GetErrorString() + ')');
      image.Dispose();
      Result := oxeIMAGE;
   end;
end;
